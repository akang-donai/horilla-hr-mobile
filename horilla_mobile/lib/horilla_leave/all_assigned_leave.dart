...
''';
        ...
        if (allLeave['name'] == leave) {
          ...
          var body = jsonEncode({
            ...
            "leave_type_ids": [leaveId],
                    ...
                    var response = await ApiClient.instance.post('/api/leave/assign-leave/?leave_type_id=$leaveId', jsonBody: jsonDecode(body));
        }
      }
    }
  }
  ...
  Future<void> getAssignedLeaveType() async {
    ...
    while (hasMoreData) {
            var response = await ApiClient.instance.get('/api/leave/assign-leave/?leave_type_id=$assignedTypeItem&page=$page');
      if (response.statusCode == 200) {
        final responseData = jsonDecode(response.body);
